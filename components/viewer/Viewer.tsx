...
export function Viewer() {
    ...
    const { hideText, judgeByGpa, semesterMixMode, collapseAllSemesters } = useOptions();
    ...
    return (
        <div>
            <p className="mt-8 text-center print:hidden flex justify-center">
                <button
                    type="button"
                    onClick={() => {
                        void load();
                ...
                {' '}
                <RelativeTime value={lastUpdated} suffix=" 更新" fallback="尚未更新" refreshIntervalMs={60_000} />
            </p>
            <div className="text-[1.2em]">
                ...
                {semesters.map((semester, index) => (
                    <SemesterSection
                        ...
                        hideText={hideText}
                        judgeByGpa={judgeByGpa}
                        collapseAll={collapseAllSemesters}
                        onTamper={tamper}
                        onUntamper={untamper}
                    />
                ))}
...
function sanitizeSemesterName(course: Course): string {
    ...
    const semesterDisplay = Number.isNaN(course.semester) ? '--' : course.semester;
